package elice.eliceauction.domain.member;
...
public class JwtFilterAuthenticationTest {
    ...
    private static final String LOGIN_FAIL_MESSAGE = "";
    @Autowired
    MockMvc mockMvc;
    ...
    @BeforeEach
    public void init(){
        memberRepository.save(Member.builder().username(USERNAME).password(delegatingPasswordEncoder.encode(PASSWORD)).name("Member1").role(Role.USER).build());
        clear();
    }
